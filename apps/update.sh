...
process_spec() {
    ...
    branch="$(sed -n 's/%global[[:space:]]\+\bbranch\b[[:space:]]\+\(.*\)/\1/p' "$1")"
    if [[ -z "$branch" ]]; then
        branch="master"
    fi

    oldCommit="$(sed -n 's/%global[[:space:]]\+\bcommit0\b[[:space:]]\+\(.*\)/\1/p' "$1")"
    IFS=$'\n' read -r -d '' -a commits <<< "$(gh api --method GET "repos/KDE/$baseName/commits?sha=$branch&per_page=50" -q '.[].sha')"
    ...
    do
        ...
        if [[ "$message" =~ ^(SVN|GIT)_SILENT.*|snapcraft: ]]; then
            continue
        fi
        break
    done
...
git diff HEAD~..HEAD
git push --quiet origin unstable

# shellcheck disable=SC2154
# shellcheck disable=SC1083
xargs -a <(parallel echo {/.} ::: "${changedSpecs[@]}") ruby ~/ruby/deps.rb --copr solopasha/kde-gear-unstable -r f41 -s
